...
## v13.9 · RECORD FORMAT / ADMIN SORT

- 공개 기록 상세보기에서 `content`의 원문 줄바꿈과 빈 줄을 유지하도록 본문 렌더러를 개선.
- `- `, `– `, `— `로 시작하는 연속 항목은 실제 목록으로 렌더링.
- 일부 SQL/기존 데이터에서 목록 앞 줄바꿈이 공백으로 합쳐진 경우에도 명시적 bullet 표식을 복구해 목록으로 표시.
- 관리자 기록 목록은 `RY-YYYY-NNN` 기록번호 기준으로 최신 연도 → 같은 연도 최신 번호 순으로 정렬.
- 관리자 본문 입력창의 줄간격/가독성 개선.
- DB schema 변경 없음. `records.content`는 계속 plain text로 저장하며, 문단은 빈 줄 1개 이상, 목록은 `- 항목` 형식으로 입력.
- 오디오 뷰어(`audio-viewer.js`, `audio-viewer.css`)는 v13.9에서 변경하지 않음.
- 패치/업데이트 이력은 별도 PATCH 문서가 아니라 이 README에 누적 기록.
